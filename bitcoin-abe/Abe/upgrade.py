...
def find_namecoin_addresses(store):
    ...
    row = store.selectrow("SELECT chain_id FROM chain WHERE chain_name = 'Namecoin'")
    if row is None:
      return
    chain_id = row[0]
    ...
    for tx_id, txout_pos, script in store.selectall("""
        ...
        pubkey_id = store.script_to_pubkey_id(store.get_chain_by_id(chain_id), store.binout(script))
