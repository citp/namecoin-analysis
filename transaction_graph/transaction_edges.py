...
#            ORDER BY cc.block_height, bt.tx_pos, txout.txout_pos""" 
    ...
    for from_pubkey_id, to_pubkey_id in store.selectall("""
          SELECT out2.pubkey_id, out1.pubkey_id
          FROM chain_candidate cc
          JOIN block_tx bt ON (cc.block_id = bt.block_id)
          JOIN txout out1 ON (bt.tx_id = out1.tx_id)
          JOIN txin ON (out1.tx_id = txin.tx_id)
          JOIN txout out2 ON (txin.txout_id = out2.tx_id)
       ...
       # height = int(height)
        ...
        #    continue
        yield (from_pubkey_id, to_pubkey_id)
...
def dump(store, logger, chain_id):
    ...
    for x in iterate_name_updates(store, logger, chain_id):
        from_pubkey_id, to_pubkey_id = x
        print repr((from_pubkey_id, to_pubkey_id))
