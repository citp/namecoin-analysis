...
Although the Namecoin ecosystem is in disrepair, nonetheless studying it is useful. Even if modern DNS meets the basic needs of its users, having a decentralized alternative provides a valuable hedge against a potentially abusive central authority. In addition to the advantages of having a decentralized DNS, namespaces like the one present in Namecoin are important to understand, because they can be used for many other types of applications and protocols. The history of attempts for PGP key directories demonstrates this. OneName, for example, utilizes a namespace for keeping track of a user's identity instead of DNS. While decentralized namespaces have many important applications, their promise hasn't been realized so far. Our work helps understand why this might be and lays the groundwork for a more rigorous approach to building such systems.
